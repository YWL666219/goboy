import pandas as pd, plotly.express as px, streamlit as st, io, json, time
from utils import *
...
# 删除了未使用的cached_statistical_analysis函数


# 删除了未使用的异步任务包装器和重计算函数


def create_chart(input_data, chart_type):
    """生成统计图表"""
    charts = {
        "bar": lambda: px.bar(x=input_data["columns"], y=input_data["data"], title="📊 柱状图分析", color=input_data["data"], color_continuous_scale="viridis"),
        "line": lambda: px.line(x=input_data["columns"], y=input_data["data"], title="📈 趋势分析", markers=True),
        "pie": lambda: px.pie(values=input_data["data"], names=input_data["columns"], title="🥧 饼图分析")
    }
    ...
    if chart_type in charts:
        ...
        df = pd.DataFrame({'项目': input_data["columns"], '数值': input_data["data"]})
        st.markdown("#### 📋 数据表格")
        st.dataframe(df, use_container_width=True, hide_index=True)
        data = input_data["data"]
        cols = st.columns(4)
        metrics = [("总计", sum(data)), ("平均值", sum(data)/len(data)), ("最大值", max(data)), ("最小值", min(data))]
...
def display_data_overview(df):
    ...
    # 基本信息
    metrics = [("总行数", len(df)), ("总列数", len(df.columns)), 
               ("数值列", len(df.select_dtypes(include=['number']).columns)), 
    ...
    # 列信息详情
    st.markdown("#### 📋 列信息详情")
    info_df = pd.DataFrame([{"列名": col, "数据类型": str(df[col].dtype), 
                            "非空值": df[col].count(), "缺失值": df[col].isnull().sum(), 
...
def data_cleaning_section(df):
    ...
    with st.expander("📊 数据质量评估", expanded=False):
        ...
        with col3:
            numeric_ratio = (len(df.select_dtypes(include=['number']).columns) / len(df.columns)) * 100
